...
def test_reprioritise_bank_statement_when_internal_age_five_mins_or_more(queue):
    ...
    bank_statements_task = TaskSubmission(provider=BANK_STATEMENTS_PROVIDER.name, user_id=234, timestamp="2025-10-20 12:01:00", metadata={ "priority": Priority.NORMAL })
    companies_house_task = TaskSubmission(provider=COMPANIES_HOUSE_PROVIDER.name, user_id=345, timestamp="2025-10-20 12:07:00", metadata={ "priority": Priority.NORMAL })
...
def test_reprioritise_bank_statement_when_internal_age_five_mins_or_more_multiuser(queue):
    ...
    ch_u2 = TaskSubmission(provider=COMPANIES_HOUSE_PROVIDER.name, user_id=234, timestamp="2025-10-20 12:01:00", metadata={ "priority": Priority.NORMAL })
    id_v_u2 = TaskSubmission(provider=ID_VERIFICATION_PROVIDER.name, user_id=234, timestamp="2025-10-20 12:06:00", metadata={ "priority": Priority.NORMAL })
    ...
    queue.enqueue(ch_u2)
    queue.enqueue(id_v_u2)
    queue.enqueue(bank_statements_u2)
